...
namespace CWM.Services
{
    public class SecurityServices
    {
        ...
        public CWM.Core.Security.AuthenticationResponse AuthenticateUser(string userName, string password, string url, ISecurityContext securityContext)
        {
            var u = new UserRepository().GetByEmailPassword(userName, SecurityUtils.GetMd5Hash(password));
            var response = new CWM.Core.Security.AuthenticationResponse();
            if (u != null)
            {
                if (!u.IsActive)
                {
